...
export function buildGrantResourceId(grantId: string): string {
  return `${encodeURIComponent(grantId)}.ttl`
}

export function buildGrantPodResourcePath(grantId: string): string {
  return `/settings/autonomy/grants/${buildGrantResourceId(grantId)}`
}

/** @deprecated Use buildGrantPodResourcePath for Pod-root paths or buildGrantResourceId for resource-base ids. */
export const buildGrantSubjectPath = buildGrantPodResourcePath
