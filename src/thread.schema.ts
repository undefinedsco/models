 ...
 *   timelines.
 * - `parent` uses the standard Solid Chat/SIOC relation `sioc:has_parent`.
 *   The parent container identifies the command surface/counterpart without
 *   duplicating parallel chat/task ownership fields.
 * - AI product runtime sessions map to Thread when they represent a concrete
 *   conversation timeline/place/run.
 * - Thread carries workspace/place relations and runtime metadata.
 * - Product/runtime-specific ids should stay in namespaced metadata such as
 *   `metadata.protocols.<apiNs>`. Do not name the generic thread id
 *   `piSessionId`, `matrixRoomId`, or `chatkitThreadId`.
 *
...
export const threadResource = podTable(
  'thread',
  {
    id: id('id').default((key: string | undefined, row?: Record<string, unknown>) => (
      renderDefaultIdTemplate('{parent.dir}/index.ttl#{key}', {
        key: resourceKey(key, 'thread'),
        row,
      })
    )),

    // Parent command surface/container. Uses the standard Solid Chat/SIOC relation.
