 ...
 * 
 * CommonJS module for compatibility with rdf-test-suite
 */

const { QueryEngine } = require('@comunica/query-sparql');
const { Store: N3Store } = require('n3');
...
module.exports = {
  ...
  async parse(queryString, options) {
    ...
    try {
      await engine.queryBindings(queryString, { sources: [n3Store] });
    } catch (e) {
      // Ignore execution errors, we just want to validate parsing
